...
const _existing = figma.currentPage.children.find(n => n.name === "Admin / Settings / Organização");
if (_existing) { _existing.remove(); console.log("Removed existing frame: Admin / Settings / Organização"); }

// 
// STEP 1 — Frame principal 1440x880
// 
const screen = figma.createFrame();
screen.name = "Admin / Settings / Organização";
...
if (bc) await tryFont(bc, "Organização");
...
const _screen = figma.currentPage.children.find(n => n.name === "Admin / Settings / Organização");
if (!_screen) throw new Error("Frame 'Admin / Settings / Organização' nao encontrado — execute STEP 0-4 primeiro");
...
```
Frame criado: Admin / Settings / Organização
Sidebar ativo: "Settings"
Referência: 3134:3649
