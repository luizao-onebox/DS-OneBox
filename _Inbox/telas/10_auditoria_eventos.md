...
const _existing = figma.currentPage.children.find(n => n.name === "Admin / Auditoria / Ações & Eventos");
if (_existing) { _existing.remove(); console.log("Removed existing frame: Admin / Auditoria / Ações & Eventos"); }

// 
// STEP 1 — Frame principal 1440x880
// 
const screen = figma.createFrame();
screen.name = "Admin / Auditoria / Ações & Eventos";
...
if (bc) await tryFont(bc, "Ações & Eventos");
...
const _screen = figma.currentPage.children.find(n => n.name === "Admin / Auditoria / Ações & Eventos");
if (!_screen) throw new Error("Frame 'Admin / Auditoria / Ações & Eventos' nao encontrado — execute STEP 0-4 primeiro");
...
```
Frame criado: Admin / Auditoria / Ações & Eventos
Sidebar ativo: "Auditoria"
Referência: 3134:3585
...
Feed/Event Item: 3415:36213 — COMPONENT_SET, variantes Status=APROVAÇÃO/REJEIÇÃO/ESCALADO/REVISÃO
```
